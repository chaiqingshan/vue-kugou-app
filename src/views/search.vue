<template>
	<div>
		...
		<div class="search-list" v-show="togglePanel">
			...
			<mint-cell v-for="(item,index) in hotList" :title="item" @click.native="replaceInput(index)"></mint-cell>
		</div>

		<div class="song-list" v-show='!togglePanel'>
		...
		methods: {
			...
			search(){
				...
				this.$http.get('http://cs003.m2828.com/apis/proxy.php?val=&url1=http://mobilecdn.kugou.com/api/v3/search/song?keyword=' + this.keyword + '&page=1&pagesize=30&url2=').then(res=>{
					...
					this.songList=[...list.map(({filename,hash})=>({filename,hash}))];
					Indicator.close();
				});
			}
		}
